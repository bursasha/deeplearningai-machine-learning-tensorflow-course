...
def create_train_val_dirs(root_path):
  ...
  # Directories for dataset separation for training, validation, etc.
  ...
  for main_dir in main_dirs:
      ...
      for pet_dir in pet_dirs:
        path = os.path.join(root_path + "/" + main_dir, pet_dir)
        os.makedirs(path)
...
def split_data(SOURCE_DIR, TRAINING_DIR, VALIDATION_DIR, SPLIT_SIZE):
  ...
  start_images_count = len(shuffled_images)
  current_images_count = start_images_count

  while current_images_count > 0:
    image = shuffled_images.pop()
    current_images_count -= 1

    if os.path.getsize(SOURCE_DIR + image) > 0:
      target_dir = TRAINING_DIR if round(current_images_count / start_images_count, 2) >= round(1 - SPLIT_SIZE, 2) \
                                else VALIDATION_DIR
